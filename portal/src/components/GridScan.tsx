...
export const GridScan: React.FC<GridScan.Props> = ({
  enableWebcam = false,
  showPreview = false,
  ...
  }, [snapBackDelay, scanOnClick, sensitivity]);
...
function smoothDampVec2(
  ...
  const out = adjustedTarget.clone().add(change.add(temp).multiplyScalar(exp));
...
function smoothDampFloat(
  ...
  const temp = current - change;
  const temp2 = (velRef.v + omega * change) * deltaTime;
  velRef.v = (velRef.v - omega * temp2) * exp;
  let out = temp + (change + temp2) * exp;
